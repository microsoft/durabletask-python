...
class AsyncExportHistoryClient:
    ...
    def __init__(
        self,
        durable_task_client: client_module.AsyncTaskHubGrpcClient,
        writer: HistoryWriter,
    ) -> None:
        self._client = durable_task_client
        self._writer = writer
    ...
    async def create_job(
        ...
        *,
        ...
        await self._client.signal_entity(
            entity_id,
            ExportJobEntity.OP_CREATE,
            input={"config": config.to_dict(), "created_at": created_at.isoformat()},
        )
        logger.info(
            "Submitted export job %r; orchestrator instance ID will be %s",
            resolved_job_id, orchestrator_instance_id_for(resolved_job_id),
    ...
    async def get_job(self, job_id: str) -> ExportJobDescription | None:
        """Look up an export job by ID, or return None if it does not exist."""
        entity_id = entities.EntityInstanceId(ENTITY_NAME, job_id)
        meta = await self._client.get_entity(entity_id, include_state=True)
        if meta is None:
            return None
        state = meta.get_typed_state()
        if not isinstance(state, dict):
    ...
    async def list_jobs(
        ...
        status_filter = set(query.status) if query.status else None
        for meta in await self._client.get_all_entities(entity_query):
            if meta.id.entity != ENTITY_NAME.lower():
                continue
            state = meta.get_typed_state()
            if not isinstance(state, dict):
            ...
            try:
                ...
                logger.warning(
                    "list_jobs: skipping export-job entity %r; state did not match "
                    "the current schema (%s)", meta.id.key, ex)
                continue
            if status_filter is None or desc.status in status_filter:
                yield desc
    ...
    async def wait_for_job(
        ...
        *,
        ...
        """Wait asynchronously until a job is terminal."""
        if timeout <= 0:
            raise ValueError("timeout must be positive")
        if poll_interval <= 0:
            raise ValueError("poll_interval must be positive")
        ...
        last: ExportJobDescription | None = None
        while True:
            desc = await self.get_job(job_id)
            if desc is not None:
                last = desc
                if desc.status in _TERMINAL_STATUSES:
                    return desc
    ...
    async def delete_job(self, job_id: str) -> None:
        ...
        entity_id = entities.EntityInstanceId(ENTITY_NAME, job_id)
        orch_instance_id = orchestrator_instance_id_for(job_id)
        await self._client.signal_entity(entity_id, ExportJobEntity.OP_DELETE)
        try:
            await self._client.terminate_orchestration(orch_instance_id, recursive=True)
        except grpc.RpcError as ex:
            if _grpc_status(ex) != grpc.StatusCode.NOT_FOUND:
                raise
        try:
            await self._client.wait_for_orchestration_completion(
                orch_instance_id, timeout=_DELETE_WAIT_TIMEOUT_SECONDS)
        except TimeoutError:
            logger.warning(
                "Export job %r orchestrator %r did not terminate within %ss; "
            ...
            if _grpc_status(ex) != grpc.StatusCode.NOT_FOUND:
                raise
        try:
            await self._client.purge_orchestration(orch_instance_id, recursive=True)
        except grpc.RpcError as ex:
            if _grpc_status(ex) != grpc.StatusCode.NOT_FOUND:
                raise
    ...
    def get_job_client(self, job_id: str) -> "AsyncExportHistoryJobClient":
        """Return an asynchronous per-job façade for *job_id*."""
        return AsyncExportHistoryJobClient(self, job_id)

    @property
    def writer(self) -> HistoryWriter:
        """Get the history writer used by this client."""
        return self._writer


class AsyncExportHistoryJobClient:
    """Asynchronous per-job façade returned by AsyncExportHistoryClient."""

    def __init__(self, parent: AsyncExportHistoryClient, job_id: str) -> None:
        if not job_id:
            raise ValueError("job_id must be a non-empty string")
        self._parent = parent
        self._job_id = job_id
    ...
    @property
    def job_id(self) -> str:
        """Get this export job's ID."""
        return self._job_id

    @property
    def orchestrator_instance_id(self) -> str:
        """Get the driving orchestrator's ID."""
        return orchestrator_instance_id_for(self._job_id)

    async def describe(self) -> ExportJobDescription | None:
        """Fetch the latest description, or None if the job is missing."""
        return await self._parent.get_job(self._job_id)

    async def wait(
        self, *, timeout: float = 300.0, poll_interval: float = 1.0,
    ) -> ExportJobDescription:
        """Wait asynchronously until this job is terminal."""
        return await self._parent.wait_for_job(
            self._job_id, timeout=timeout, poll_interval=poll_interval)

    async def delete(self) -> None:
        """Delete this export job."""
        await self._parent.delete_job(self._job_id)
